...
# 用法: <path_to/>ipgw_core "examples/ensure_conn.rb" >> ipgw.log

exit if `tasklist`.downcase.scan("ipgw_core").length > 1 # 不能打开多个以避免混乱
...
begin
  ...
  $stderr.puts '用户数据读取失败, 或未存储相关数据. 请在设置完毕退出后重启本程序'
...
def checkConn(timeout=5, site='www.baidu.com')
  ...
  $stderr.puts '网络连接失败 - ' + @ipgw.errMsg # GBK
end
